...
- 配列アクセスとジェネリクスの型情報が表示される
- インポートされたクラスの完全修飾名が表示される
- スコープ外の変数参照で適切なエラーメッセージが表示される
...
- パッケージ宣言にホバーした際に情報が表示される（M4.5以降で対応予定）
- インポート文にホバーした際に情報が表示される（M4.5以降で対応予定）
- インターフェース名にホバーした際に情報が表示される（M4.5以降で対応予定）
- インターフェースメソッドにホバーした際に情報が表示される（M4.5以降で対応予定）
- プロパティにホバーした際に型情報が表示される
- フィールドにホバーした際に情報が表示される
- コンストラクタにホバーした際に情報が表示される
- ローカル変数にホバーした際に型情報が表示される
- パラメータにホバーした際に型情報が表示される
- Spockのgivenブロックにホバーした際に情報が表示される（M6.4で対応予定）
- Spockのwhenブロックにホバーした際に情報が表示される（M6.4で対応予定）
- Spockのthenブロックにホバーした際に情報が表示される（M6.4で対応予定）
- SpockのMockにホバーした際に情報が表示される（M6.4で対応予定）
- SpockのStubbing演算子>>にホバーした際に情報が表示される（M6.4で対応予定）
- 型名（Long）にホバーした際に情報が表示される
- 文字列リテラルにホバーした際に情報が表示される（M9.5で対応予定）
- 数値リテラルにホバーした際に情報が表示される（M9.5で対応予定）
- クロージャにホバーした際に情報が表示される（M9.5で対応予定）
- defキーワードにホバーした際に情報が表示される（M9.5で対応予定）
- extendsキーワードにホバーした際に情報が表示される（M9.5で対応予定）
- メソッド呼び出しにホバーした際に情報が表示される
- 名前付き引数にホバーした際に情報が表示される（M9.5で対応予定）
